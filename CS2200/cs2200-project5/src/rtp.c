...
packet_t *packetize(char *buffer, int length, int *count) {
    int num_packet = length / MAX_PAYLOAD_LENGTH;
    int temp_packet = MAX_PAYLOAD_LENGTH;

    if (length % MAX_PAYLOAD_LENGTH != 0) {
        temp_packet = length % MAX_PAYLOAD_LENGTH;
        num_packet++;
    }
    packet_t *packets = malloc(sizeof(packet_t) * (size_t)num_packet);
    for (int i = 0; i < num_packet; i++) {
        if (i + 1 == num_packet) {
            packets[i].type = LAST_DATA;
            packets[i].payload_length = temp_packet;
        } else {
            packets[i].type = DATA;
        ...
        do {
            ...
            */
            ...
            if (packet.type == DATA || packet.type == LAST_DATA) {
                packet_t *temp_packet = malloc(sizeof(packet_t));

                if (packet.checksum == checksum(packet.payload, packet.payload_length)) {
                    temp_packet->type = ACK;
                    if (buffer == NULL) {
                        buffer = malloc((unsigned long)packet.payload_length);
                    } else {
                        buffer = realloc(buffer, (unsigned long)(buffer_length + packet.payload_length));
                    }
                    ...
                    temp_packet->type = NACK;
                }
            net_send_packet(connection->net_connection_handle, temp_packet);
            free(temp_packet);
