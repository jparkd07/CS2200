...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        pte_t *page_entry = (pte_t*)(mem + frame_table[victim_pfn].process->saved_ptbr * PAGE_SIZE) + frame_table[victim_pfn].vpn;
        if (page_entry->dirty == 1) {
            swap_write(page_entry, mem + page_entry->pfn * PAGE_SIZE);
            stats.writebacks++;
        }
        page_entry->valid=0;
